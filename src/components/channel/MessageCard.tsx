...
export const MessageCard: React.FC<MessageCardProps> = ({
  ...
  if (isPronoMessage) {
    ...
    let analysisText = '';
    if (mainText.includes('💡 Analyse:')) {
      const parts = mainText.split('💡 Analyse:');
    ...
    return (
      <div className="w-full flex justify-center group animate-fade-in relative px-0 sm:px-1 my-2">
        {/* Swipe Reply Icon */}
        {swipeX > 20 && (
          <div 
            className="absolute left-2 top-1/2 -translate-y-1/2 text-green-500 z-0 flex flex-col items-center justify-center message-swipe-overlay"
        ...
        <div 
          className="w-full max-w-md relative z-10 message-swipe-content flex flex-col items-center"
        ...
        >
          {/* Pronostic Card */}
          <div className="rounded-2xl p-3 sm:p-4 bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 border border-emerald-500/40 shadow-xl w-full text-white overflow-hidden">
            {/* Top Header Row */}
            <div className="flex items-center justify-between border-b border-emerald-500/20 pb-2 mb-2.5 gap-1.5">
              <div className="flex items-center gap-1.5 min-w-0 flex-1">
                ...
                <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[8px] sm:text-[9px] font-black uppercase tracking-wider shrink-0 ${
              ...
              <span className={`text-[9px] sm:text-[10px] font-extrabold uppercase px-2 sm:px-2.5 py-0.5 rounded-full border shadow-sm shrink-0 whitespace-nowrap ${statusStyle}`}>
                {statusBadge}
              </span>
            </div>
            ...
            {fullHeaderSubtitle && (
              <div className="text-[10px] sm:text-[11px] font-semibold text-slate-400 mb-1 flex items-center gap-1 min-w-0">
            ...
            <div className="mb-2.5">
              <h4 className="text-sm sm:text-base font-black text-white tracking-tight flex items-center gap-1.5 break-words">
                <span className="shrink-0">⚽</span>
                <span className="break-words">{matchName}</span>
              </h4>
            </div>
            ...
            {!isPending && explanationText && (
              <div className="rounded-xl p-2.5 bg-slate-950/50 border border-emerald-500/20 mb-2.5 text-xs">
                <p className={`leading-relaxed text-[10px] sm:text-[11px] font-medium break-words ${isWon ? 'text-emerald-300' : 'text-slate-300'}`}>
                  {explanationText}
                </p>
              </div>
            )}
            ...
            {!isPending && (message.pronoStreak !== undefined || message.pronoWinRate !== undefined) && (
              <div className="rounded-xl px-2.5 sm:px-3 py-1.5 sm:py-2 bg-gradient-to-r from-amber-500/10 via-amber-600/15 to-amber-500/10 border border-amber-500/30 text-amber-300 flex items-center justify-center gap-1.5 text-[10px] sm:text-[11px] font-bold shadow-sm mb-2 text-center flex-wrap">
                <span className="shrink-0">🔥</span>
                <span>
                  {message.pronoStreak ? `Série : ${message.pronoStreak} gagnant${message.pronoStreak > 1 ? 's' : ''}` : 'Nouvelle série'}
                  {message.pronoWinRate !== undefined && ` · Taux : ${message.pronoWinRate}%`}
                </span>
              </div>
            )}
            ...
            {analysisText && (
              <div className="bg-emerald-950/40 rounded-xl p-2.5 border border-emerald-500/20 text-xs mb-2">
                ...
                <p className="leading-relaxed text-slate-300 italic text-[10px] sm:text-[11px] break-words">{analysisText}</p>
              </div>
            )}

            {/* Double Timestamp Footer */}
            <div className="text-right text-[9px] sm:text-[10px] text-slate-400 font-medium pt-1 border-t border-white/5 truncate">
          ...
          <div className="flex items-center justify-end w-full space-x-3 mt-1 px-1 transition-opacity opacity-0 group-hover:opacity-100">
            <button
              onClick={(e) => {
                e.stopPropagation();
                onReply();
