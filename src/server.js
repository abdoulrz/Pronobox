...
const FOOTBALL_RSS_SOURCES = [
  { url: 'https://rmcsport.bfmtv.com/rss/football/', name: 'RMC Sport', sourceUrl: 'https://rmcsport.bfmtv.com' },
  { url: 'https://dwh.lequipe.fr/api/edito/rss?path=/Football/Transferts-football/', name: "L'Equipe", sourceUrl: 'https://www.lequipe.fr' },
  { url: 'https://www.francetvinfo.fr/sports/foot.rss', name: 'France Info', sourceUrl: 'https://www.francetvinfo.fr' },
  { url: 'https://www.lemonde.fr/football/rss_full.xml', name: 'Le Monde', sourceUrl: 'https://www.lemonde.fr' }
];
...
// If a channel's owner ref exists but wasn't populated, resolve it.
// If the owner user was deleted (orphan ref after DB reset), re-assign
// to the first populated member — the channel creator was always a member.
async function ensureChannelOwner(ch) {
  if (ch.owner && ch.owner.username) return ch.owner;
  // owner is a raw ObjectId that wasn't populated, or null
  const rawOwnerId = ch.owner?._id || ch.owner;
  if (rawOwnerId) {
    try {
      const ownerUser = await User.findById(rawOwnerId).select('_id username avatar');
      if (ownerUser) {
        ch.owner = { _id: ownerUser._id, username: ownerUser.username, avatar: ownerUser.avatar };
        return ch.owner;
      }
      // Owner ID exists but user was deleted (orphan ref after DB reset).
      // Re-assign to the first available member (creator is always member[0]).
      if (Array.isArray(ch.members) && ch.members.length > 0) {
        const firstMember = ch.members[0];
        const memberId = firstMember?._id || firstMember;
        const memberUser = await User.findById(memberId).select('_id username avatar');
        if (memberUser) {
          await Channel.updateOne({ _id: ch._id }, { $set: { owner: memberUser._id } });
          ch.owner = { _id: memberUser._id, username: memberUser.username, avatar: memberUser.avatar };
          console.log(`Channel "${ch.name}": orphan owner fixed → ${memberUser.username}`);
          return ch.owner;
        }
      }
    } catch (e) { /* owner ID invalid, fall through */ }
  }
  // Owner truly missing — log it but don't invent one
  if (!rawOwnerId) {
    console.warn(`Channel "${ch.name}" (${ch._id}) has no owner set.`);
  }
  return ch.owner;
}
...
async function autoGenerateDebatesFromNews() {
  try {
    ...
    // Programmatic cleanup of expired debates (not favorited and either past expiresAt or older than 24 hours)
    const cutoffDate = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const deleteResult = await Debate.deleteMany({
    ...
    if (deleteResult.deletedCount > 0) {
      console.log(`[AutoDebate] Programmatically cleaned up ${deleteResult.deletedCount} expired auto-generated debates`);
    }

    const existingAutoDebates = await Debate.countDocuments({ isAutoGenerated: true, isFavorite: { $ne: true } });
...
const initializeMockData = async () => {
  try {
    ...
    if (pronosboxUser) {
      ...
      if (doobiChannel) {
        let needsFix = !doobiChannel.owner;
        if (doobiChannel.owner && !needsFix) {
          const currentOwner = await User.findById(doobiChannel.owner);
          if (!currentOwner) needsFix = true;
        }
        if (needsFix) {
          doobiChannel.owner = pronosboxUser._id;
          if (!doobiChannel.members.some(m => String(m) === String(pronosboxUser._id))) {
